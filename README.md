...
The quality feature drives YouTube's own Settings-menu UI only - the old `setPlaybackQuality()` JS API is silently ignored for most viewers, and calling it with a tier above the free maximum popped the Premium upsell, so it was removed. The automation runs at most once per video URL and only once the `<video>` element actually has media data (`readyState >= 1`); player states like "buffering"/"cued" are not trusted, and any ad state counts as "not ready" - so the addon never touches the player during ad-blocker stalls (the black 0:00 window some blockers produce). A capped retry plus the 5-second heartbeat re-check means the preferred quality still applies once the real video starts, however long the stall lasted. While the automation runs, the menu is hidden twice over: a CSS class on `<html>` covering both classic and role-based menu containers, plus an inline `display:none` on the exact menu root being driven (class-name independent). The menu is closed while still hidden, then everything is un-hidden. The "Skip Premium quality options" toggle controls Premium handling: when checked (default), rows whose label contains the (untranslated) brand term "Premium" are skipped, the closest lower free tier is picked, and any upsell dialog is hidden invisibly; when unchecked, Premium rows are treated as selectable and preferred (for members), and dialogs are left alone - the addon never verifies membership. If no suitable tier exists at or below the preferred resolution, the automation leaves the video on Auto instead of clicking anything. The automation is skipped entirely when the player is off-screen.
...
- **The watch-page title is restored through its heading container** (`ytd-watch-metadata h1` / `#title h1`): on every scan, if the container's visible text is empty or still translated, the original is written into whichever child element actually renders (old `yt-formatted-string` or newer view-model nodes) - so the title stays visible even when YouTube swaps or rebuilds the title element between layouts. Feed/Shorts title nodes are healed the same way, including unmarked empty ones whose video ID can be resolved.
- **Titles are restored** in feed/search cards keyed off `#video-title` (including Shorts and notifications), and on the `.ytp-title-link` overlay shown on embedded players - with the video ID taken from each card's own link. Shorts-page titles (`yt-shorts-video-title-view-model`), home Shorts shelf and channel Shorts tab cards (`.shortsLockupViewModelHostEndpoint`) and newer view-model grids (`h3[title] > a > span[role='text']`) are covered too; on the Shorts page the video ID is taken from the URL. Description snippets are scanned site-wide (`.metadata-snippet-text` / `#description-text`) and matched to their video via the nearest `/watch` or `/shorts` link.
