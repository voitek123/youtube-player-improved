...
Mute hover previews works at three layers so no unmuted audio can escape: a MutationObserver mutes any preview `<video>` the instant it is added to the DOM (before playback can start); a capture-phase `play` listener mutes it at the exact moment playback begins; and a 1-second polling backstop catches anything the first two miss. A "preview" is any `<video>` that is NOT the current page's real player (watch `#movie_player`, Shorts `#shorts-player`, channel featured `#c4-player`, miniplayer) - this matters because YouTube now plays hover-preview audio through pooled SECONDARY players, which older container-class exclusions skipped. Muting sets both `muted = true` and `volume = 0`, and a `volumechange` listener pushes the level back down if a preview tries to unmute itself. Iframes that YouTube itself creates for hover previews (referrer = youtube.com) are muted too, while legitimate third-party embeds are never touched.
