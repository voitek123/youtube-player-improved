...
The addon is built to be near-invisible to the page. There are no polling sweeps and no observers that do per-node subtree work: the Premium-upsell and hover-preview observers each do a single cheap `matches()` per added node, and the translation observer only schedules a debounced scan. Heavy full-document translation scans are deferred until ~2.5 s after navigation (after thumbnails paint), are never run from the periodic heartbeat, and afterwards run only on settled mutations (900 ms debounce) with a fetch budget and a synchronous cache so already-correct titles cost zero. Background cadence is a 10 s heartbeat of cheap self-gating checks. With toggles off the addon is essentially idle; with toggles on, all work is event- or interval-bounded, so YouTube stays as smooth as stock.
...
Mute hover previews never writes `video.muted` / `video.volume` on previews. When a hover-preview node appears (detected by a cheap `matches()` per added node - no subtree scans), the addon waits for the preview's `<video>` to fire `playing` (real playback; 1200 ms fallback), then clicks YouTube's own corner mute button ONCE - targeted precisely by volume/mute/sound keywords in its aria-label/class, with the old "first non-CC/close button" only as fallback - and runs a single 300 ms verify-and-correct pass. Clicking only at playback start avoids the premature control clicks that made YouTube re-init (restart) the preview. Each preview instance is processed at most once (WeakSet), so it can never oscillate.
...
- **Deferred & throttled scanning:** the first full translation pass runs ~2.5 s after navigation (after thumbnails paint), never from the periodic heartbeat; afterwards a 900 ms-debounced observer scan keeps things current. Each scan processes at most a small budget of fresh (uncached) titles so YouTube never rate-limits the addon; anything left over is retried by up to two targeted re-scans. Restored titles are kept in a synchronous cache so repeat scans skip already-correct nodes with zero async work.
