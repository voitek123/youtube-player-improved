...
- **Mute hover previews** – videos that start playing when you hover a thumbnail stay muted. Previews are muted the instant their video element appears or begins playback (MutationObserver + capture-phase play listener), so no unmuted, choppy audio can escape; a slow polling backstop covers edge cases. Videos inside real players (watch, Shorts, channel featured, miniplayer, embeds) are never touched. On by default.
...
Mute hover previews works at three layers so no unmuted audio can escape: a MutationObserver mutes any preview `<video>` the instant it is added to the DOM (before playback can start); a capture-phase `play` listener mutes it at the exact moment playback begins (this removes the old choppy-audio window that 600ms polling left); and a 1-second polling backstop catches anything the first two miss. Any `<video>` that lives outside the real player containers (`#movie_player`, `#shorts-player`, `#c4-player`, miniplayer, `.html5-video-player`) is treated as a hover/inline preview and muted; videos inside those players are never touched.
