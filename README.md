...
- **Captions & subtitles control** – keeps captions always off (or always on), overriding YouTube's auto-detected preference so they don't switch themselves on when a video loads. Only acts once the player is ready and never toggles twice in one pass, so the control bar no longer flickers at load. Also works on embedded YouTube players on other sites.
...
- **No translations** – keeps titles, descriptions and chapters in their original language instead of YouTube's automatic translation, on every YouTube subpage: the watch-page title, tab title and tooltips; feed, search, channel and playlist titles (including the home Shorts shelf, channel Shorts tabs and the new grid layouts); the Shorts watch-page title; notifications; the watch-page description and feed/search description snippets; chapter names (control-bar chip, seekbar hover pill, chapters panel); and the title overlay shown on embedded players. Title writes are race-safe and self-healing. YouTube UI chrome (view counts, buttons, hints) follows the interface language and is intentionally left untouched.
...
The quality feature drives YouTube's own Settings-menu UI only - the old `setPlaybackQuality()` JS API is silently ignored for most viewers, and calling it with a tier above the free maximum popped the Premium upsell, so it was removed. The automation runs at most once per video URL and only after the player reports real video data (`readyState` / player state), with a short capped retry if the player is still initializing - touching the menu earlier could stall the initial stream request (black 0:00 player) on recent Firefox builds. While it runs, the menu is hidden twice over: a CSS class on `<html>` covering both classic and role-based menu containers, plus an inline `display:none` on the exact menu root being driven (class-name independent). The menu is closed while still hidden, then everything is un-hidden. The "Skip Premium quality options" toggle controls Premium handling: when checked (default), rows whose label contains the (untranslated) brand term "Premium" are skipped, the closest lower free tier is picked, and any upsell dialog is hidden invisibly; when unchecked, Premium rows are treated as selectable and preferred (for members), and dialogs are left alone - the addon never verifies membership. If no suitable tier exists at or below the preferred resolution, the automation leaves the video on Auto instead of clicking anything. The automation is skipped entirely when the player is off-screen.
...
Captions control waits for a ready player, then uses EITHER the player API (`isSubtitlesOn`/`toggleSubtitlesOn`) OR the visible CC button as a fallback when the API is missing - never both in the same pass, so the captions state can't oscillate and re-render the control bar.
...
- **Titles are restored** on the watch page (including tab title and tooltips), in feed/search cards keyed off `#video-title` (including Shorts and notifications), and on the `.ytp-title-link` overlay shown on embedded players - with the video ID taken from each card's own link. Shorts-page titles (`yt-shorts-video-title-view-model`), home Shorts shelf and channel Shorts tab cards (`.shortsLockupViewModelHostEndpoint`) and newer view-model grids (`h3[title] > a > span[role='text']`) are covered too; on the Shorts page the video ID is taken from the URL. Description snippets are scanned site-wide (`.metadata-snippet-text` / `#description-text`) and matched to their video via the nearest `/watch` or `/shorts` link.
- **Title writes are race-safe and self-healing:** before an async fetch resolves, the addon remembers what the title node displayed; the write only lands if the node is still connected to the page and still shows that exact text (i.e. YouTube hasn't reused it for another video). Restored nodes are marked with the video ID; on later scans, a marked node whose text YouTube re-rendered back to a translated or empty value is repaired from the cached original.
