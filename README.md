...
- **Default volume level** – sets the volume when a video actually starts playing (and once per new Short). It opens a ~3.5-second protection window with scheduled re-asserts to guarantee your chosen level beats YouTube's delayed volume restore (especially common with ad blockers); after the window the volume is fully under your control.
- **Block volume scroll** – scrolling the mouse wheel over the player's volume control no longer changes the volume.
- **Mute hover previews** – videos that start playing when you hover a thumbnail stay muted. Previews are muted the instant their video element appears or begins playback (MutationObserver + capture-phase play listener), using both `muted` and `volume=0` so YouTube cannot undo it; YouTube's pooled secondary preview players and YouTube-created preview iframes are covered too, while the real player (watch, Shorts, channel featured, miniplayer, embeds) is never touched. On by default.
...
The addon is built to be near-invisible to the page: observers are event-driven and narrow (the hover-preview observer reacts only to added `<video>` nodes; the Shorts-loop observer stops at the first nested video), background cadence is slow (10 s heartbeat, 2 s preview poll, 500 ms translation debounce), and repeat work is skipped via synchronous caches so already-restored titles cost zero. There are no per-scroll subtree scans and no volume tug-of-war, so YouTube stays as smooth as stock.
...
The default volume level is applied through capture-phase listeners, anchoring the real enforcement to the `playing` event (which fires when media actually starts, safely bypassing ad-blocker stalls). For ~3.5 seconds after that, scheduled re-asserts at +0.2s, +1s, +2s, and +3s explicitly overwrite the volume, guaranteeing it beats YouTube's delayed state restore. A `volumechange` listener also re-asserts the level during the window if YouTube tries to change it. After the window there is deliberately no enforcement and the volume belongs to the user. Toggling the feature or moving the slider while a video is loaded applies the new level immediately.
...
Mute hover previews works at three cheap layers so no unmuted audio can escape: a MutationObserver reacts only to added `<video>` nodes and mutes them before playback can start; a capture-phase `play` listener mutes a preview at the exact moment playback begins; and a 2-second polling backstop catches stragglers. A "preview" is any `<video>` that is NOT the current page's real player (watch `#movie_player`, Shorts `#shorts-player`, channel featured `#c4-player`, miniplayer) - this matters because YouTube now plays hover-preview audio through pooled SECONDARY players, which older container-class exclusions skipped. Muting sets both `muted = true` and `volume = 0`; muted previews are tracked in a WeakSet so the `volumechange` handler only ever touches known previews and no-ops when already silent (no tug-of-war, no wasted writes). Iframes that YouTube itself creates for hover previews (referrer = youtube.com) are muted too, while legitimate third-party embeds are never touched.

The Shorts loop fix strips the `loop` attribute from `<video>` elements and blocks the `.loop` JS property. Its observer stops at the first nested video per inserted node (a 500 ms sweep catches any extras), avoiding full subtree scans on every insertion.
